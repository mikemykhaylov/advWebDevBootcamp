...
].filter((val) => !Object.values(val).includes(null));

const svgSize = 800;
const padding = 50;
const xExtent = d3.extent(regionData, (d) => d.extremePovertyRate);
const xScale = d3
  .scaleLinear()
  .domain(xExtent)
  .range([padding, svgSize - padding]);
const yExtent = d3.extent(regionData, (d) => d.urbanPopulationRate);
const yScale = d3
  .scaleLinear()
  .domain(yExtent)
  .range([svgSize - padding, padding]);
const growthRateExtent = d3.extent(regionData, (d) => d.growthRate);
const colorScale = d3
  .scaleLinear()
  .domain(growthRateExtent)
  .range(['#192a56', '#4cd137']);
...
d3.select('svg')
  ...
  .attr('cy', (d) => yScale(d.urbanPopulationRate))
  .attr('fill', (d) => colorScale(d.growthRate))
  .attr('r', (d) => birthsScale(d.medianAge));
...
d3.select('svg')
  ...
  .attr('font-size', '2em')
  .style('text-anchor', 'middle')
  .text('Extreme poverty vs urban population:');
