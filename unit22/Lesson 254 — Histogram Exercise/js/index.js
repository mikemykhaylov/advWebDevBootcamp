...
].filter((val) => val.medianAge !== null);
const svgSize = 800;
const svgPadding = 40;
const barPadding = 1;
const minAge = d3.min(regionData, (d) => d.medianAge);
const maxAge = d3.max(regionData, (d) => d.medianAge);
function updateGraph(desiredBins) {
  const xScale = d3
    .scaleLinear()
  ...
  const histogram = d3
    ...
    .thresholds(desiredBins)
    .value((d) => d.medianAge);
  const bins = histogram(regionData);
  const yScale = d3
    .scaleLinear()
    .domain([0, d3.max(bins, (d) => d.length)])
    .range([svgSize - svgPadding, svgPadding]);
  const xAxis = d3.axisBottom(xScale).ticks(bins.length - 1);
  ...
  newBars
    .merge(bars)
    .select('rect')
    .attr('height', (d) => svgSize - yScale(d.length) - svgPadding)
  ...
  d3.select('svg')
    ...
    .call(yAxis);
  d3.select('h4').text(`Number of bins: ${bins.length}`);
}
...
d3.select('svg')
  ...
  .style('text-anchor', 'middle')
  .text('Frequency:');
d3.select(window).on('load', () => updateGraph(1));
d3.select('#bins').on('input', () => {
  const desiredBins = parseInt(d3.event.target.value, 10);
  updateGraph(desiredBins);
});
