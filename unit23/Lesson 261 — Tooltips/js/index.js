...
const svgSize = 800;
const padding = 40;
const yExtent = d3.extent(birthData2011, (d) => d.lifeExpectancy);
const yScale = d3
  .scaleLinear()
  .domain(yExtent)
  .range([svgSize - padding, padding]);
const xExtent = d3.extent(birthData2011, (d) => d.births / d.population);
const xScale = d3
  .scaleLinear()
  .domain(xExtent)
  .range([padding, svgSize - padding]);
const densityExtent = d3.extent(birthData2011, (d) => d.population / d.area);
const colorScale = d3
  .scaleLinear()
  .domain(densityExtent)
  .range(['#4cd137', '#c23616']);
const birhsExtent = d3.extent(birthData2011, (d) => d.births);
const birthsScale = d3
  .scaleLinear()
  .domain(birhsExtent)
  .range([2, 40]);
const xAxis = d3
  .axisBottom(xScale)
  .tickSize(-svgSize + 2 * padding)
  .tickSizeOuter(0);
const yAxis = d3
  .axisLeft(yScale)
  .tickSize(-svgSize + 2 * padding)
...
d3.select('svg')
  .append('g')
  .attr('transform', `translate(0, ${svgSize - padding})`)
  .call(xAxis);
d3.select('svg')
  .append('g')
...
d3.select('svg')
  ...
  .attr('y', svgSize / 2)
  .attr('dy', '-1.6em')
  .attr('transform', `rotate(-90, ${padding}, ${svgSize / 2})`)
  .style('text-anchor', 'middle')
